...
class SGDOptimizer(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float) -> None:
        ...
        Note:
            ...
            - While you are not allowed to use the class above, it might be extremely beneficial to look at its code when implementing step function.
    ...
    def step(self, closure=None):  # noqa: E251
        """
        Performs a step of gradient descent. You should loop through each parameter, and update its value based on its gradient, value and learning rate.
        ...
        """
        for i in self.param_groups: #Superclass stores parameters in self.param_groups
            lr = i['lr']
        
            for p in i['params']:
                if p.grad is not None:
                    p.data -= lr * p.grad.data
                #p.data is parameter val
                #p.grad.data is gradient of param
